...
-- => 15
--          -reducer        -init   -iterable     -output
foldLeft :: (b -> a -> b)   -> b    -> List a     -> b
foldLeft _ b Nil      = b  --if empty list return init
...
map f (x:.xs) = (f x) :. (map f xs)
  --map f (x:xs) = [f x | x <- xs]  
--map f a = foldLeft (\acc x -> acc ++ f x) Nil
...
filter f a = foldLeft (\acc val -> if f val then val :. acc else acc) Nil a
--filter f a = foldRight (\x _ -> if f x == True then (x.:) else id) Nil
...
(++) ::
  ...
  -> List a
(++) = flip (foldRight (:.))
-- flips because the init should be the left argument ? 
...
seqOptional (x:.xs) = applyOptional (mapOptional (:.) x) (seqOptional xs)
